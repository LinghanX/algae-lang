...
(define/rec interleave-helper
  (lambda (x left right)
    (if (null? right)
        (cons (append left (cons x right)) null)
        (cons (append left (cons x right))
              (interleave-helper x
                                 (append left (cons (car right) null))
                                 (cdr right))))))
;; tests
...
;; range-helper : Nat Nat -> (Listof Nat)
;; returns a list of numbers from m (inclusive) to n (exclusive)
(define/rec range-helper
  (lambda (n m)
    (if (= n m)
        null
        (cons m (range-helper n (+ 1 m))))))
;; tests
